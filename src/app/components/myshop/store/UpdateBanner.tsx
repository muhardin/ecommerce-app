...
const UpdateBanner: React.FC<ModalProps> = ({
  isOpen,
  closeModal,
  banners,
}) => {
  ...
  useEffect(() => {
    ...
    setContent(banners?.text || "");
    setClicked(banners?.href || "");
    if (banners?.type == "slide") {
      setType(1);
    } else {
      setType(0);
    }
  }, [banners?.href, banners?.text, banners?.title, banners?.type]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    axios
      .post(
        `${process.env.SERVER_ENDPOINT}/api/myshop-board/banner/update/post/` +
          banners?.id,
        formData,
        { headers }
      )
