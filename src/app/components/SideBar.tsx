...
const SideBar = ({ value, onValueChange }: Props) => {
  const toggleValue = () => {
    onValueChange(!value);
  };
  ...
  const [isToggledOrder, setIsToggledOrder] = useState(false);
  ...
  //     scale: isToggled ? 0.8 : 1,
  //   });
  //   setIsToggledMenu(!isToggledMenu); // Toggle the state
  // };

  // const toggleMenuOrder = async () => {
  //   setIsToggledOrder(!isToggledMenu); // Toggle the state
  // };
  const [isToggledMenu, setIsToggledMenu] = useState(false);
  // setIsToggledMenu(false);
  // console.log(toggleNavVisibility);
  // console.log(value);
  return (
    <>
      <div className="fixed top-30 left-0 md:relative">
        <div className={` `}>
          <nav
            ref={ref}
            id="default-sidebar"
            className={`bg-bodyColor ${
              !value ? "hidden" : "block"
            } md:block top-0 left-0 z-40 w-64 h-screen transition-transform ${
              !value ? "-translate-x-full sm:translate-x-0" : ""
            }`}
            aria-label="Sidenav"
          >
            <div className="overflow-y-auto py-5 px-3 h-full border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
              <ul className="space-y-2 tracking-wide mt-0">
                <li>
                  <Link
                    aria-current="page"
                    onClick={() => {
                ...
                <li>
                    ...
                    <a
                      ...
                      className={`px-4 py-3 flex items-center space-x-4 rounded-md ${
                        isToggled
                          ? " text-white bg-gradient-to-r from-sky-600 to-cyan-400 "
                          : "text-gray-600"
                      }  group justify-between duration-400 transition-all`}
                    >
                      <div className=" justify-start flex flex-row gap-2">
                        <svg
                  ...
                  <div className="transition-all duration-300">
                    <ul
                      id="dropdown-pages"
                      className={`${!isToggled ? "hidden" : ""} py-2 space-y-2`}
                    >
                      <li>
                        <a
                          href="#"
                  ...
                  <Link
                    ...
                    className={`${
                      pathname == "/buyer/order"
                        ? " text-white bg-gradient-to-r from-sky-600 to-cyan-400 "
                        : "text-gray-600"
                    } px-4 py-3 flex items-center space-x-4 rounded-md text-gray-600 group active`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5"
