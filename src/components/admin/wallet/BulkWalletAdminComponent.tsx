...
const BulkWalletAdminComponent: React.FC<ModalProps> = ({
  isBatch,
  closeBatch,
  checkedItems,
}) => {
  ...
  const onNote = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setCheckNote(e.target.value);
  };
  ...
  const submitUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.SERVER_ENDPOINT}/api/admin/wallet/withdraw/multi-status`,
        {
          id: checkedItems,
          note: checkNote,
          status: checkStatus,
                ...
                <div className="flex md:flex-row flex-col justify-between mr-20">
                  <div>
                    <h4 className="text-xl font-medium dark:text-gray-300">
                      Update Selected Withdraw
                    </h4>
                    <p className="mb-0 text-sm dark:text-gray-300">
                      Apply changes to the selected Withdraw from the list
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="absolute inset-0 overflow-scroll -mr-[17px] -mb-[17px]">
                  <form className="block" onSubmit={submitUpdate}>
                    ...
                    <div className="h-screen bg-white w-full p-12">
                      ...
                      <div className="grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6 items-center w-full">
                        <label className="block text-gray-800 dark:text-gray-400 col-span-4 sm:col-span-2 font-medium text-sm">
                          Product Status
                        </label>
                        <div className="col-span-8 sm:col-span-4">
                          <div className="mb-2 w-full">
                            <div className="w-full">
                              ...
                              >
                                ...
                                <option selected={checkStatus == "3"} value={3}>
                                  Rejected
                                </option>
                      ...
                      <div
                        className={`${
                          checkStatus == "3" ? "block" : "hidden"
                        } grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6 items-center w-full`}
                      >
                        <label className="block text-gray-800 dark:text-gray-400 col-span-4 sm:col-span-2 font-medium text-sm">
                          Rejected Note
                        </label>
                          ...
                          <div className="mb-2 w-full">
                            <div className="w-full h-full">
                              <textarea
                                ...
                                value={checkNote}
                                required={checkStatus == "3"}
                                className="textarea textarea-error w-full h-full"
                                placeholder="Notes"
                              ></textarea>
                            </div>
                          </div>
...
export default BulkWalletAdminComponent;
