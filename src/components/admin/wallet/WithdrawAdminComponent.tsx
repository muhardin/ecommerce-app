...
import BulkWalletAdminComponent from "./BulkWalletAdminComponent";
import AddWithdrawAdminComponent from "./AddWithdrawAdminComponent";

const WithdrawAdminComponent = () => {
  ...
  const url = `${process.env.SERVER_ENDPOINT}/api/admin/wallet/withdraw?page=${currentPage}&status=${statusFilter}`;
  const {
    data: productsItems,
    isLoading,
    isValidating,
  ...
  const toggleChecked = () => {
    ...
    const filteredItems = productsItems.data.filter(
      (item: Withdraw) => item.status < 2
    );
    filteredItems.forEach((item: Products) => {
      ...
      .then(async (result) => {
        ...
        if (result.isConfirmed) {
          ...
          if (act == "accept") {
            ...
            try {
              const response = await axios.post(
                `${process.env.SERVER_ENDPOINT}/api/admin/wallet/withdraw/multi-done`,
                {
                  id: checkedIds,
                },
                {
                    ...
                    <button
                      ...
                      className={`${
                        checkedItemIds.length > 0
                          ...
                          : "bg-sky-300 disabled opacity-50"
                      } align-bottom inline-flex items-center justify-center leading-5 transition-colors duration-150 font-medium focus:outline-none px-4 py-2 rounded-lg text-sm text-white border border-transparent  w-full h-12   btn-red relative z-0`}
                      type="button">
                      <span className="mr-2">
                        <svg
            ...
            <div className="w-full overflow-x-auto">
              <table className="w-full whitespace-nowrap">
                ...
                <tbody className="bg-white divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800 text-gray-800 dark:text-gray-400">
                  {productsItems ? (
                    productsItems.data.map((item: Withdraw, index: number) => (
                      <tr className="" key={item.id}>
                        ...
                        <td className="px-4 py-2">
                          <div className="">
                            <div className="">{item.bank_name}</div>
                            <div className="">{item.bank_account_name}</div>
                            <div className="">{item.bank_account_number}</div>
...
export default WithdrawAdminComponent;
