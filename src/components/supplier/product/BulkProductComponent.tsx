...
const BulkProductComponent: React.FC<ModalProps> = ({
  isBatch,
  closeBatch,
  checkedItems,
}) => {
  ...
  const submitUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.SERVER_ENDPOINT}/api/supplier-board/product/update-multi`,
        {
          id: checkedItems,
          isNew: checkedNew,
          isPublish: checked,
          supplier_id: valSuppG,
          category: valCatG,
        },
        {
          headers: { Authorization: `Bearer ${session?.bearer}` },
...
export default BulkProductComponent;
